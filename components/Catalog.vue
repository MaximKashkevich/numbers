    ...
    <div class="flex gap-[20px] items-center justify-between flex-wrap">
      ...
      <div>
        ...
        <div @click="toggleEmirateDropdown" class="relative">
          ...
          <ul v-if="isEmirateDropdownOpen"
            class="absolute mt-[5px] w-full bg-white border border-[#BFBFBF] rounded-md shadow-lg max-h-60 overflow-scroll">
      ...
      <div>
        ...
        <div @click="toggleCodeDropdown" class="relative">
          ...
          <ul v-if="isCodeDropdownOpen"
            class="absolute mt-[5px] w-full bg-white border border-[#BFBFBF] rounded-md shadow-lg max-h-60 overflow-scroll">
            <li v-for="code in codes" @click="handleCodesChange(code.name)" :key="code.id"
              class="py-2 px-4 hover:bg-gray-200 cursor-pointer font-medium">
              {{ code.name }}
            </li>
          </ul>
      ...
      <div>
        ...
        <div @click="toggleSortDropdown" class="relative">
          <button
            class="mt-[10px] text-[16px] leading-[19.2px] text-left w-[220px] bg-[#FAFAFA] border border-[#BFBFBF] rounded-[25px] py-[15px] px-[20px] flex items-center justify-between">
            {{ selectedSort || 'Latest' }} <!-- Начальное значение -->
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform duration-200"
              :class="{ 'rotate-180': isSortDropdownOpen }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          <ul v-if="isSortDropdownOpen"
            class="absolute mt-[5px] w-full bg-white border border-[#BFBFBF] rounded-md shadow-lg max-h-60 overflow-scroll">
            <li class="py-2 px-4 hover:bg-gray-200 cursor-pointer font-medium">
              Latest
            </li>
            <li class="py-2 px-4 hover:bg-gray-200 cursor-pointer font-medium">
              Earliest
